...
int tag[10000];
int result[100];
int af_weight[4][4096][4096];
double af_last_weight[4096][10];
double af_last_bias[10];
double bn_weight[4][4096];
double bn_bias[4][4096];
int dot[4][4096][100];
double f_dot[5][4096][100];

int mnist_nn(int start_index);
...
int main(){
    ...
    for(int i=0; i<NUM_ITERATIONS/NUM_ITERATION_PER_EPOCH; i++){
    	printf("----------Epoch%d----------\n", i+1);
    	mnist_nn(i*NUM_ITERATION_PER_EPOCH);
    	for(int j=0; j<NUM_ITERATION_PER_EPOCH; j++){
    		printf("%3d result:%d, answer:%d\n", j, result[j], tag[j+i*NUM_ITERATION_PER_EPOCH]);
    		if(result[j] != tag[j+i*NUM_ITERATION_PER_EPOCH]){
    			hw_err_count++;
    		}
    	}
